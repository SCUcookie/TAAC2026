...
class RankMixerBlock(nn.Module):
    ...
    def __init__(
        ...
        norm_type: str = 'layernorm',
        ffn_type: str = 'gelu',
        legacy_ffn_names: bool = False,
    ) -> None:
        super().__init__()
        self.T = n_total
        self.D = d_model
        self.mode = mode
        self.legacy_ffn_names = legacy_ffn_names
        ...
        if legacy_ffn_names and ffn_type == 'gelu':
    ...
    def forward(self, Q: torch.Tensor) -> torch.Tensor:
        ...
        # Per-token FFN
        x = self.norm(Q_hat)
        if self.legacy_ffn_names:
            x = self.fc1(x)
            x = F.gelu(x)
            x = self.dropout(x)
            Q_e = self.fc2(x)
            Q_e = self.dropout(Q_e)
        else:
            Q_e = self.ffn(x)
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        norm_type: str = 'layernorm',
        seq_topk_mode: str = 'recent',
        legacy_mixer_ffn_names: bool = False,
    ) -> None:
        super().__init__()
        self.num_sequences = num_sequences
        self.num_queries = num_queries
        ...
        self.mixer = RankMixerBlock(
            ...
            norm_type=norm_type,
            ffn_type=ffn_type,
            legacy_ffn_names=legacy_mixer_ffn_names,
        )
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        legacy_user_dense_proj: bool = False,
        legacy_mixer_ffn_names: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.legacy_user_dense_proj = legacy_user_dense_proj
        self.legacy_mixer_ffn_names = legacy_mixer_ffn_names
        ...
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                norm_type=norm_type,
                seq_topk_mode=seq_topk_mode,
                legacy_mixer_ffn_names=legacy_mixer_ffn_names,
            )
    ...
    def _prepare_tokens(self, inputs: ModelInput) -> Tuple[torch.Tensor, list, list, list]:
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            if self.legacy_user_dense_proj:
                user_dense_tokens = F.silu(self.user_dense_proj(inputs.user_dense_feats)).unsqueeze(1)
            else:
