...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        self.use_action_weight = args.use_action_weight  # 接收启用开关
        self.action_weights = torch.tensor([1.0, 2.0], dtype=torch.float32).to(self.dev)
    ...
    def forward(
        ...
        loss_mask = (next_mask == 1).float().to(self.dev)
        if self.use_action_weight:
            action_type = torch.clamp(next_action_type, 0, 1).long().to(self.dev)
            action_weight = self.action_weights[action_type]
            loss_mask = loss_mask.float() * action_weight  # 转 float 避免布尔运算问题
        else:
    ...
    # 1. 生成布尔类型的有效掩码（0表示无效，非0表示有效）
        valid_mask = loss_mask > 0  # [batch_size, maxlen]，布尔张量，维度与loss_mask一致
        valid_weights = loss_mask[valid_mask]  # [num_valid]，提取有效样本的权重（float）
        num_valid = valid_weights.size(0)
    
    # 2. 若无有效样本，直接返回0损失（避免后续计算报错）
        if num_valid == 0:
            return torch.tensor(0.0, device=seq_embs.device, requires_grad=True), 0.0, 0.0
    
    # 3. 关键修复：用布尔掩码提取有效样本的Embedding（维度变为 [num_valid, hidden_units]）
    # 注意：需要用 .float() 保持与权重的类型一致（可选，但避免后续类型警告）
        valid_seq_embs = seq_embs[valid_mask].float()  # [num_valid, D]
        valid_pos_embs = pos_embs[valid_mask].float()  # [num_valid, D]
        valid_neg_embs = neg_embs[valid_mask].float()  # [num_valid, D]
    
    # 4. 计算正样本相似度（维度匹配：[num_valid, D] × [num_valid, D] → [num_valid, 1]）
        valid_pos_logits = F.cosine_similarity(valid_seq_embs, valid_pos_embs, dim=-1).unsqueeze(-1)
    
    # 5. 计算负样本相似度（[num_valid, D] × [num_valid, D]^T → [num_valid, num_valid]）
        neg_logits = torch.matmul(valid_seq_embs, valid_neg_embs.transpose(-1, -2))
    
    # 6. 拼接logits并应用温度系数
        logits = torch.cat([valid_pos_logits, neg_logits], dim=-1)  # [num_valid, 1 + num_valid]
        logits = logits / self.temp
    
    # 7. 加权交叉熵损失（修复后维度完全匹配）
        labels = torch.zeros(logits.size(0), device=logits.device, dtype=torch.int64)
        per_sample_loss = F.cross_entropy(logits, labels, reduction='none')  # [num_valid]
        weighted_loss = (per_sample_loss * valid_weights).sum() / valid_weights.sum()  # 权重归一化
    
    # 8. 计算日志指标
        pos_sim_mean = valid_pos_logits.mean().item()
        neg_sim_mean = neg_logits.mean().item()
    
        return weighted_loss, pos_sim_mean, neg_sim_mean