...
def get_args():
    ...
    # Train params - 优化推理批次大小
    parser.add_argument('--batch_size', default=256, type=int)  # 降低推理批次以节省内存
...
def perform_python_faiss_search():
    ...
    # 优化FAISS HNSW参数以适应冷启动场景
    index = faiss.IndexHNSWFlat(vector_dim, 48)  # M=48 降低内存使用
    index.hnsw.efConstruction = 2048  # 增加构建质量
    ...
    # 设置查询参数，提高检索精度
    index.hnsw.efSearch = 800  # 增加efSearch提高冷启动场景召回率
...
def process_cold_start_feat(feat):
    """
    增强的冷启动特征处理，基于数据分析结果优化混合类型和缺失值处理
    注意：特征'111'和'115'在模型中没有对应的embedding，所以不应处理
    """
    # 基于数据分析的特征默认值映射
    feature_defaults = {
        # '111': 0,      # 超高基数特征，模型中未创建embedding，跳过处理
        # '115': 493,    # 基于分析报告的统计均值，模型中未创建embedding，跳过处理
        '102': 0,      # 高基数ID特征
        '117': 0, '118': 0, '119': 0, '120': 0, '121': 0, '122': 0  # 混合类型特征
    }
    
    processed_feat = {}
    for feat_id, feat_value in feat.items():
        # 跳过模型中未创建embedding的特征
        if feat_id in ['111', '115']:
            continue
        ...
        if type(feat_value) == list:
            ...
            for v in feat_value:
                if type(v) == str:
                    try:
                        value_list.append(int(v))  # 尝试智能转换
                    except (ValueError, TypeError):
                        value_list.append(feature_defaults.get(feat_id, 0))
                else:
                    value_list.append(v)
            processed_feat[feat_id] = value_list
        elif type(feat_value) == str:
            try:
                processed_feat[feat_id] = int(feat_value)  # 智能转换字符串数字
            except (ValueError, TypeError):
                processed_feat[feat_id] = feature_defaults.get(feat_id, 0)
        else:
            processed_feat[feat_id] = feat_value
    
    # 注释掉特征115的处理，因为模型中没有对应的embedding
    # if '115' not in processed_feat:
    #     processed_feat['115'] = feature_defaults['115']
    
    return processed_feat


def get_valid_item_pool(retrieve_id2creative_id):
    """
    构建有效物品ID池，用于在映射失败时进行替补
    
    Returns:
        valid_items: 有效物品ID列表（排除0和无效值）
    """
    valid_items = []
    for creative_id in retrieve_id2creative_id.values():
        try:
            if creative_id != "0" and int(creative_id) > 0:
                valid_items.append(creative_id)
        except (ValueError, TypeError):
            continue
    return list(set(valid_items))  # 去重

def candidate_feature_generator(candidate_path, indexer, feat_types, feat_default_value, mm_emb_dict, batch_size=10000):
    """
    内存优化的候选物品特征生成器，批量处理减少内存峰值
    
    Args:
        candidate_path: 候选物品文件路径
        indexer: 索引字典
        feat_types: 特征类型
    ...
    Yields:
        (item_ids_batch, retrieval_ids_batch, features_batch, retrieve_id2creative_id_batch)
    """
    import gc
    
    EMB_SHAPE_DICT = {"81": 32, "82": 1024, "83": 3584, "84": 4096, "85": 3584, "86": 1024}
    
    item_ids_batch, retrieval_ids_batch, features_batch = [], [], []
    retrieve_id2creative_id_batch = {}
    batch_count = 0
    ...
    with open(candidate_path, 'r') as f:
        for line in f:
            ...
            missing_fields = missing_fields - {'111', '115'}  # 排除未创建embedding的特征
            ...
            retrieval_ids_batch.append(retrieval_id)
            features_batch.append(feature)
            retrieve_id2creative_id_batch[retrieval_id] = creative_id
            
            # 当达到批次大小时，yield并清空
            if len(item_ids_batch) >= batch_size:
                batch_count += 1
                print(f"Yielding batch {batch_count}: {len(item_ids_batch)} items")
                yield (item_ids_batch, retrieval_ids_batch, features_batch, retrieve_id2creative_id_batch)
        ...
        if item_ids_batch:
            batch_count += 1
            print(f"Yielding final batch {batch_count}: {len(item_ids_batch)} items")
            yield (item_ids_batch, retrieval_ids_batch, features_batch, retrieve_id2creative_id_batch)

def get_candidate_emb(indexer, feat_types, feat_default_value, mm_emb_dict, model):
    """
    优化的候选库embedding生成，使用生成器模式减少内存使用
    ...
    # 使用USER_CACHE_PATH缓存中间结果
    cache_path = os.environ.get('USER_CACHE_PATH', './user_cache')
    os.makedirs(cache_path, exist_ok=True)
    cache_file = Path(cache_path) / 'candidate_embeddings.cache'
    
    # 检查缓存
    if cache_file.exists():
        print(f"Loading cached candidate embeddings from {cache_file}")
        with open(cache_file, 'rb') as f:
            return pickle.load(f)
    
    retrieve_id2creative_id = {}
    all_item_ids, all_retrieval_ids, all_features = [], [], []
    
    print("Processing 660K candidates with memory-optimized generator...")
    
    # 使用生成器批量处理，减少内存峰值
    for batch_idx, (item_ids_batch, retrieval_ids_batch, features_batch, id_mapping_batch) in enumerate(
        candidate_feature_generator(candidate_path, indexer, feat_types, feat_default_value, mm_emb_dict, batch_size=8000)
    ):
        # 累积数据用于最终的embedding生成
        all_item_ids.extend(item_ids_batch)
        all_retrieval_ids.extend(retrieval_ids_batch)
        all_features.extend(features_batch)
        retrieve_id2creative_id.update(id_mapping_batch)
        
        print(f"Processed batch {batch_idx + 1}: total {len(all_item_ids)} items accumulated")
        
        # 定期清理内存
        if (batch_idx + 1) % 10 == 0:
            gc.collect()
            print(f"Memory cleanup performed after batch {batch_idx + 1}")

    # 保存候选库的embedding和ID映射
    print(f"Generating embeddings for {len(all_item_ids)} candidates...")
    model.save_item_emb(all_item_ids, all_retrieval_ids, all_features, os.environ.get('EVAL_RESULT_PATH'))
    ...
    with open(cache_file, 'wb') as f:
        pickle.dump(retrieve_id2creative_id, f)
...
def infer():
    """
    优化的主推理函数：针对冷启动场景的高效推理
    
    基于数据分析的优化策略：
    1. 智能处理混合数据类型和特征115缺失问题
    2. 内存优化的66万候选物品处理
    3. 优化的FAISS检索参数配置
    4. 缓存机制减少重复计算
    5. 性能监控和内存管理
    ...
    test_loader = DataLoader(
        ...
        num_workers=min(4, os.cpu_count()//2), collate_fn=test_dataset.collate_fn,
    ...
    if args.use_python_faiss:
        ...
        ann_cmd = (
            ...
            + " --faiss_M=48 --faiss_ef_construction=2048 --query_ef_search=800"  # 基于冷启动场景优化FAISS参数
    ...
    if args.use_multistage_recall:
        ...
        # 改进的多阶段召回：使用有效物品池进行替补
        print("Stage 2: Improved re-ranking with fallback...")
        
        # 构建有效物品池用于替补
        valid_item_pool = get_valid_item_pool(retrieve_id2creative_id)
        print(f"DEBUG: Built valid item pool with {len(valid_item_pool)} items")
        ...
        for user_idx, top100_items in enumerate(top100s_retrieved):
            ...
            # 处理前30个候选以获得更多选择
            for item_retrieval_id in top100_items[:30]:
                total_mappings += 1
                item_creative_id = retrieve_id2creative_id.get(int(item_retrieval_id), None)
                
                if item_creative_id is None:
                    mapping_failures += 1
                    # 映射失败时从有效池中随机选择
                    if valid_item_pool:
                        import random
                        fallback_id = random.choice(valid_item_pool)
                        if fallback_id not in valid_items:
                            valid_items.append(fallback_id)
                    continue
                ...
                try:
                    ...
                    if item_id_int > 0 and item_creative_id not in valid_items:
            ...
            # 如果有效项目不足10个，从有效池中补充
            while len(valid_items) < 10 and valid_item_pool:
                import random
                fallback_id = random.choice(valid_item_pool)
                if fallback_id not in valid_items:
                    valid_items.append(fallback_id)
                    ...
                    # 如果随机选择重复，强制添加不重复的
                    remaining_pool = [item for item in valid_item_pool if item not in valid_items]
                    if remaining_pool:
                        valid_items.append(remaining_pool[0])
                    else:
                        break  # 池子用完了
            
            # 最后如果还不够，用0填充
            while len(valid_items) < 10:
                valid_items.append("0")
            
            top10s_reranked.append(valid_items[:10])
        
        # 输出映射质量统计
        if total_mappings > 0:
            failure_rate = mapping_failures / total_mappings
            print(f"DEBUG: ID mapping failure rate: {failure_rate:.2%} ({mapping_failures}/{total_mappings})")
        ...
        for user_idx, user_candidates in enumerate(tqdm(top10s_retrieved, desc="Converting IDs")):
            ...
            for item_idx, item_retrieval_id in enumerate(user_candidates[:10]):
                total_mappings += 1
                creative_id = retrieve_id2creative_id.get(int(item_retrieval_id), None)
                ...
                if creative_id is None:
                    ...
                    if valid_item_pool:
                        import random
                        creative_id = random.choice(valid_item_pool)
                    else:
                        creative_id = "0"
            ...
            while len(user_results) < 10:
                if valid_item_pool:
                    import random
                    user_results.append(random.choice(valid_item_pool))
                else:
        ...
        if total_mappings > 0:
            failure_rate = mapping_failures / total_mappings
            print(f"DEBUG: Single-stage ID mapping failure rate: {failure_rate:.2%} ({mapping_failures}/{total_mappings})")
    ...
    print(f"候选物品数量: 660,000")
    print(f"平均每用户推理时间: {total_time/len(user_list)*1000:.2f}ms")
...
if __name__ == "__main__":
    if is_taac2026_data_dir(os.environ.get("EVAL_DATA_PATH")):
        from taac2026_infer import infer as taac2026_infer

        print(taac2026_infer())
        raise SystemExit(0)
