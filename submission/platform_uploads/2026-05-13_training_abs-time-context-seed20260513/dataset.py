...
def build_time_context_features(
    timestamps: "npt.NDArray[np.int64]",
    utc_offset_hours: int = 8,
) -> "npt.NDArray[np.float32]":
    """Build compact absolute-time context features from Unix timestamps.

    The competition data is Tencent China traffic, so the default alignment
    uses UTC+8. Output columns are:
    [sin(hour), cos(hour), sin(weekday), cos(weekday), is_weekend, log_hour]
    """
    ts = np.asarray(timestamps, dtype=np.int64)
    safe_ts = np.maximum(ts, 0)
    local_ts = safe_ts + int(utc_offset_hours) * 3600
    hours = ((local_ts // 3600) % 24).astype(np.float32)
    weekdays = ((local_ts // 86400 + 3) % 7).astype(np.float32)

    hour_angle = (2.0 * np.pi * hours) / 24.0
    weekday_angle = (2.0 * np.pi * weekdays) / 7.0

    feats = np.zeros((ts.shape[0], TIME_CONTEXT_DIM), dtype=np.float32)
    feats[:, 0] = np.sin(hour_angle).astype(np.float32)
    feats[:, 1] = np.cos(hour_angle).astype(np.float32)
    feats[:, 2] = np.sin(weekday_angle).astype(np.float32)
    feats[:, 3] = np.cos(weekday_angle).astype(np.float32)
    feats[:, 4] = (weekdays >= 5).astype(np.float32)
    feats[:, 5] = np.log1p(hours).astype(np.float32)
    feats[ts <= 0] = 0.0
    return feats
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        time_context_utc_offset_hours: int = 8,
    ) -> None:
        """
        Args:
        ...
        self.time_context_utc_offset_hours = int(time_context_utc_offset_hours)
        ...
        self._buf_time_context_dense = np.zeros((B, TIME_CONTEXT_DIM), dtype=np.float32)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        if self.use_absolute_time_context:
            time_context = self._buf_time_context_dense[:B]
            time_context[:] = build_time_context_features(
                timestamps,
                utc_offset_hours=self.time_context_utc_offset_hours,
            )
            result['time_context_dense_feats'] = torch.from_numpy(time_context.copy())
        seq_primary_values: Dict[str, "npt.NDArray[np.int64]"] = {}
        seq_all_values: Dict[str, "npt.NDArray[np.int64]"] = {}
