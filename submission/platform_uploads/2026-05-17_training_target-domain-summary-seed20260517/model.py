...
class ModelInput(NamedTuple):
    ...
    xdomain_dense_feats: Optional[torch.Tensor] = None
    seq_inter_time_buckets: Optional[dict] = None  # {domain: tensor [B, L]}
    seq_time_deltas: Optional[dict] = None         # {domain: tensor [B, L]}
    seq_inter_time_deltas: Optional[dict] = None   # {domain: tensor [B, L]}
    timestamp: Optional[torch.Tensor] = None       # tensor [B], Unix timestamp
...
class RMSNorm(nn.Module):
    """Root-mean-square normalization with LayerNorm-compatible signature."""
...
def make_norm(d_model: int, norm_type: str = 'layernorm') -> nn.Module:
    """Create a normalization layer from a small stable option set."""
    if norm_type == 'rms':
...
def make_ffn(
    ...
    """Create a residual-block FFN body that maps D -> D."""
    if ffn_type == 'swiglu':
...
class HighCardinalityEmbedding(nn.Module):
    """Multi-hash embedding for features too large for a full table."""
    ...
    def __init__(
        ...
        self.embs = nn.ModuleList([
            nn.Embedding(self.num_buckets + 1, emb_dim, padding_idx=0)
            for _ in range(self.num_hashes)
        ])
        seeds = torch.arange(1, self.num_hashes + 1, dtype=torch.long) * 1000003
        self.register_buffer('hash_seeds', seeds, persistent=False)

    def forward(self, ids: torch.Tensor) -> torch.Tensor:
        ids = ids.long()
        safe_ids = ids.clamp(min=0)
        mask = (safe_ids != self.padding_idx).unsqueeze(-1)
        pieces = []
        for i, emb in enumerate(self.embs):
            seed = self.hash_seeds[i]
            hashed = ((safe_ids * 1103515245 + seed) % self.num_buckets) + 1
            hashed = torch.where(safe_ids == self.padding_idx, torch.zeros_like(hashed), hashed)
            pieces.append(emb(hashed))
        out = torch.stack(pieces, dim=0).mean(dim=0)
        return out * mask.to(out.dtype)


def make_feature_embedding(
    vocab_size: int,
    emb_dim: int,
    emb_skip_threshold: int = 0,
    high_cardinality_mode: str = 'zero',
    hash_num_buckets: int = 1 << 20,
    hash_num_hashes: int = 2,
) -> Optional[nn.Module]:
    """Create a feature embedding or return None for zero-skip behavior."""
    vs = int(vocab_size)
    if vs <= 0:
        return None
    is_high = emb_skip_threshold > 0 and vs > emb_skip_threshold
    if is_high:
        if high_cardinality_mode == 'hash':
            return HighCardinalityEmbedding(
                vocab_size=vs,
                emb_dim=emb_dim,
                num_buckets=hash_num_buckets,
                num_hashes=hash_num_hashes,
                padding_idx=0,
            )
        return None
    return nn.Embedding(vs + 1, emb_dim, padding_idx=0)
...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        if ln_mode in ['pre', 'post']:
            self.norm_q = make_norm(d_model, norm_type)
            self.norm_kv = make_norm(d_model, norm_type)
...
class RankMixerBlock(nn.Module):
    ...
    def __init__(
        ...
        hidden_mult: int = 4,
        dropout: float = 0.0,
        mode: str = 'full',  # 'full' | 'ffn_only' | 'learned' | 'none'
        norm_type: str = 'layernorm',
        ffn_type: str = 'gelu',
    ) -> None:
        super().__init__()
        ...
        if mode == 'full':
            ...
            self.token_norm = make_norm(d_model, norm_type)
            self.token_mlp = nn.Sequential(
                nn.Linear(n_total, n_total * hidden_mult),
                nn.GELU(),
                nn.Dropout(dropout),
                nn.Linear(n_total * hidden_mult, n_total),
                nn.Dropout(dropout),
            )

        # Per-token FFN (shared parameters) — used by both 'full' and 'ffn_only'
        self.norm = make_norm(d_model, norm_type)
        self.ffn = make_ffn(d_model, hidden_mult, dropout, ffn_type)
        # Post-LN after residual to stabilize stacked block outputs
        self.post_norm = make_norm(d_model, norm_type)
    ...
    def forward(self, Q: torch.Tensor) -> torch.Tensor:
        ...
        # Token Mixing (parameter-free rewire), learned token-axis MLP, or identity
        if self.mode == 'full':
            Q_hat = self.token_mixing(Q)
        elif self.mode == 'learned':
            token_x = self.token_norm(Q)
            token_delta = self.token_mlp(token_x.transpose(1, 2)).transpose(1, 2)
            Q_hat = Q + token_delta
        else:  # 'ffn_only'
            Q_hat = Q
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.global_info_norm = make_norm(global_info_dim, norm_type)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.norm1 = make_norm(d_model, norm_type)
        self.norm2 = make_norm(d_model, norm_type)
...
class LongerEncoder(nn.Module):
    ...
    def __init__(
        ...
        ffn_type: str = 'gelu',
        norm_type: str = 'layernorm',
        seq_topk_mode: str = 'recent',
    ) -> None:
        super().__init__()
        self.top_k = top_k
        self.causal = causal
        self.seq_topk_mode = seq_topk_mode
        ...
        # FFN (Pre-LN + residual)
        self.ffn_norm = make_norm(d_model, norm_type)
        self.ffn = make_ffn(d_model, hidden_mult, dropout, ffn_type)
    ...
    def _gather_semantic_top_k(
        self,
        x: torch.Tensor,
        key_padding_mask: torch.Tensor,
        target_query: Optional[torch.Tensor],
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Select top_k valid tokens by similarity to the target query."""
        if target_query is None:
            return self._gather_top_k(x, key_padding_mask)

        B, L, D = x.shape
        device = x.device
        query = target_query.mean(dim=1) if target_query.dim() == 3 else target_query
        score = (F.normalize(x, dim=-1) * F.normalize(query, dim=-1).unsqueeze(1)).sum(dim=-1)
        score = score.masked_fill(key_padding_mask, float('-inf'))
        k = min(self.top_k, L)
        _, top_idx = torch.topk(score, k=k, dim=1)
        top_idx, _ = torch.sort(top_idx, dim=1)

        if k < self.top_k:
            pad = top_idx.new_zeros(B, self.top_k - k)
            indices = torch.cat([top_idx, pad], dim=1)
            new_padding_mask = torch.cat([
                torch.zeros(B, k, dtype=torch.bool, device=device),
                torch.ones(B, self.top_k - k, dtype=torch.bool, device=device),
            ], dim=1)
        else:
            indices = top_idx
            new_padding_mask = torch.gather(key_padding_mask, 1, indices)
    ...
    def forward(
        ...
        if L > self.top_k:
            ...
            # 1. Extract recent or semantically relevant top_k tokens as query
            if self.seq_topk_mode == 'semantic':
                q, new_mask, q_pos_indices = self._gather_semantic_top_k(
...
def create_sequence_encoder(
    ...
    if encoder_type == 'swiglu':
        return SwiGLUEncoder(d_model, hidden_mult, dropout, norm_type)
    elif encoder_type == 'transformer':
        return TransformerEncoder(d_model, num_heads, hidden_mult, dropout, ffn_type, norm_type)
    elif encoder_type == 'longer':
        return LongerEncoder(
            d_model, num_heads, top_k, hidden_mult, dropout, causal,
            ffn_type=ffn_type, norm_type=norm_type, seq_topk_mode=seq_topk_mode)
    else:
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        use_target_aware_attn: bool = False,
        use_cross_domain_attn: bool = False,
        ffn_type: str = 'gelu',
        norm_type: str = 'layernorm',
        seq_topk_mode: str = 'recent',
    ) -> None:
        ...
        self.num_queries = num_queries
        self.num_ns = num_ns
        self.use_target_aware_attn = use_target_aware_attn
        self.use_cross_domain_attn = use_cross_domain_attn
        ...
        if use_target_aware_attn:
            ...
            self.target_fuse = nn.ModuleList([
                nn.Sequential(
        ...
        if use_cross_domain_attn:
            self.domain_attns = nn.ModuleList([
                CrossAttention(
                    d_model=d_model,
                    num_heads=num_heads,
                    dropout=dropout,
    ...
    def forward(
        ...
        for i in range(S):
            ...
            if self.use_target_aware_attn and target_tokens is not None:
                target_q = self.target_query_proj(target_tokens)
                target_ctx = self.target_attns[i](
                    target_q, next_seqs[i], next_masks[i],
                    rope_cos=rc, rope_sin=rs,
                )
                target_ctx = target_ctx.mean(dim=1, keepdim=True).expand_as(decoded_q_i)
                gate_in = torch.cat([decoded_q_i, target_ctx], dim=-1)
                gate = self.target_fuse[i](gate_in)
                decoded_q_i = decoded_q_i + gate * target_ctx
            decoded_qs.append(decoded_q_i)

        if self.use_cross_domain_attn and S > 1:
            all_domain_q = torch.cat(decoded_qs, dim=1)
            refined_qs = []
            global_mean = all_domain_q.mean(dim=1)
            for i in range(S):
                domain_ctx = self.domain_attns[i](
                    decoded_qs[i], all_domain_q, None,
                )
                local_mean = decoded_qs[i].mean(dim=1)
                gate = self.domain_gates[i](torch.cat([local_mean, global_mean], dim=-1))
                refined_qs.append(decoded_qs[i] + gate.unsqueeze(-1) * (domain_ctx - decoded_qs[i]))
            decoded_qs = refined_qs
...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
        ...
        for vs, offset, length in feature_specs:
            embs.append(make_feature_embedding(
                int(vs), emb_dim, emb_skip_threshold, high_cardinality_mode,
                hash_num_buckets, hash_num_hashes))
...
class DenseFeatureTokenizer(nn.Module):
    """Split scalar dense features and wide pre-trained vectors into NS tokens."""

    def __init__(
        self,
        dense_schema_entries: Optional[List[Tuple[int, int, int]]],
        dense_dim: int,
        d_model: int,
        wide_dense_threshold: int = 32,
        use_wide_dense_tokens: bool = False,
        dense_scalar_transform: str = 'none',
        dropout: float = 0.0,
        norm_type: str = 'layernorm',
    ) -> None:
        super().__init__()
        self.dense_schema_entries = dense_schema_entries or []
        self.dense_dim = dense_dim
        self.use_wide_dense_tokens = use_wide_dense_tokens
        self.dense_scalar_transform = dense_scalar_transform
        self.scalar_slices: List[Tuple[int, int]] = []
        self.wide_slices: List[Tuple[int, int, int]] = []

        for fid, offset, length in self.dense_schema_entries:
            if use_wide_dense_tokens and length > wide_dense_threshold:
                self.wide_slices.append((fid, offset, length))
            else:
                self.scalar_slices.append((offset, length))

        known_dim = sum(length for _, _, length in self.dense_schema_entries)
        if dense_dim > 0 and (not self.dense_schema_entries or known_dim != dense_dim):
            self.scalar_slices = [(0, dense_dim)]
            self.wide_slices = []

        scalar_dim = sum(length for _, length in self.scalar_slices)
        self.has_scalar = scalar_dim > 0
        self.num_tokens = (1 if self.has_scalar else 0) + len(self.wide_slices)
        ...
        self.wide_projs = nn.ModuleList([
            nn.Sequential(
                make_norm(length, 'layernorm'),
                nn.Linear(length, d_model),
                nn.SiLU(),
                nn.Dropout(dropout),
                make_norm(d_model, norm_type),
            )
            for _, _, length in self.wide_slices
        ])

    def _transform_scalar(self, x: torch.Tensor) -> torch.Tensor:
        if self.dense_scalar_transform == 'signlog':
    ...
    def forward(self, dense_feats: torch.Tensor) -> torch.Tensor:
        ...
        if self.has_scalar:
            scalar_parts = [
                ...
                for offset, length in self.scalar_slices
            ]
            scalar = torch.cat(scalar_parts, dim=-1)
            scalar = self._transform_scalar(scalar)
            tokens.append(F.silu(self.scalar_proj(scalar)).unsqueeze(1))
        for (fid, offset, length), proj in zip(self.wide_slices, self.wide_projs):
            del fid
            wide = dense_feats[:, offset:offset + length]
            tokens.append(proj(wide).unsqueeze(1))
        if not tokens:
            return dense_feats.new_zeros(dense_feats.shape[0], 0, 0, dtype=torch.float)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_target_aware_attn: bool = False,
        use_cross_domain_attn: bool = False,
        use_abs_time_features: bool = False,
        seq_topk_mode: str = 'recent',
        ffn_type: str = 'gelu',
        norm_type: str = 'layernorm',
        random_id_mask_prob: float = 0.0,
    ) -> None:
        super().__init__()
        ...
        self.xdomain_dense_dim = xdomain_dense_dim
        self.use_inter_time_buckets = use_inter_time_buckets
        self.use_continuous_time = use_continuous_time
        self.high_cardinality_mode = high_cardinality_mode
        self.hash_num_buckets = hash_num_buckets
        self.hash_num_hashes = hash_num_hashes
        self.use_target_aware_attn = use_target_aware_attn
        self.use_cross_domain_attn = use_cross_domain_attn
        self.use_abs_time_features = use_abs_time_features
        self.seq_topk_mode = seq_topk_mode
        self.ffn_type = ffn_type
        self.norm_type = norm_type
        self.random_id_mask_prob = random_id_mask_prob
        ...
        if ns_tokenizer_type == 'group':
            ...
            self.user_ns_tokenizer = GroupNSTokenizer(
                ...
                hash_num_buckets=hash_num_buckets,
                hash_num_hashes=hash_num_hashes,
                norm_type=norm_type,
            )
        ...
        if self.has_user_dense:
            self.user_dense_tokenizer = DenseFeatureTokenizer(
                dense_schema_entries=user_dense_schema_entries,
                dense_dim=user_dense_dim,
                d_model=d_model,
                wide_dense_threshold=wide_dense_threshold,
                use_wide_dense_tokens=use_wide_dense_tokens,
                dense_scalar_transform=dense_scalar_transform,
                dropout=dropout_rate,
                norm_type=norm_type,
            )
            self.num_user_dense_tokens = self.user_dense_tokenizer.num_tokens
        else:
        ...
        self.has_xdomain_dense = use_xdomain_features and xdomain_dense_dim > 0
        if self.has_xdomain_dense:
            self.xdomain_dense_proj = nn.Sequential(
                nn.Linear(xdomain_dense_dim, d_model),
                make_norm(d_model, norm_type),
            )

        if self.use_abs_time_features:
            ...
            self.abs_time_norm = make_norm(d_model, norm_type)

        # Total NS token count
        self.num_ns = (
            num_user_ns
            + self.num_user_dense_tokens
            + num_item_ns
            + (1 if self.has_item_dense else 0)
            + (1 if self.has_xdomain_dense else 0)
            + (1 if self.use_abs_time_features else 0)
        )
        ...
        def _make_seq_embs(vocab_sizes):
            ...
            for vs in vocab_sizes:
                embs_raw.append(make_feature_embedding(
                    int(vs), emb_dim, emb_skip_threshold, high_cardinality_mode,
                    hash_num_buckets, hash_num_hashes))
        ...
        if num_time_buckets > 0:
            ...
            if use_inter_time_buckets:
                self.inter_time_embedding = nn.Embedding(num_time_buckets, d_model, padding_idx=0)
        if use_continuous_time:
            self.time_mlp = nn.Sequential(
                nn.Linear(2 if use_inter_time_buckets else 1, d_model),
                nn.SiLU(),
                nn.Linear(d_model, d_model),
            )
        ...
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                use_target_aware_attn=use_target_aware_attn,
                use_cross_domain_attn=use_cross_domain_attn,
                ffn_type=ffn_type,
                norm_type=norm_type,
                seq_topk_mode=seq_topk_mode,
            )
        ...
        # One target-conditioned history summary per sequence domain.
        self.domain_summary_query = nn.Linear(d_model, d_model)
        self.domain_summary_key = nn.Linear(d_model, d_model)
        self.domain_summary_value = nn.Linear(d_model, d_model)
        self.domain_summary_norm = make_norm(d_model, norm_type)
        self.domain_summary_gate = nn.Sequential(
            nn.Linear(d_model * 2, d_model),
            nn.Sigmoid(),
        )
        ...
        self.output_proj = nn.Sequential(
            nn.Linear((num_queries + 1) * self.num_sequences * d_model, d_model),
            make_norm(d_model, norm_type),
        )
    ...
    def _init_params(self) -> None:
        ...
        for domain in self.seq_domains:
            for emb in self._seq_embs[domain]:
                self._init_embedding_module(emb)
        ...
        if self.num_time_buckets > 0:
            ...
            if self.use_inter_time_buckets:
                nn.init.xavier_normal_(self.inter_time_embedding.weight.data)
                self.inter_time_embedding.weight.data[0, :] = 0
        if self.use_abs_time_features:
            nn.init.xavier_normal_(self.abs_hour_embedding.weight.data)
            nn.init.xavier_normal_(self.abs_weekday_embedding.weight.data)

    @staticmethod
    def _init_embedding_module(module: nn.Module) -> None:
        if isinstance(module, nn.Embedding):
            nn.init.xavier_normal_(module.weight.data)
            module.weight.data[0, :] = 0
        elif isinstance(module, HighCardinalityEmbedding):
            for emb in module.embs:
    ...
    def _build_abs_time_token(
        self,
        timestamp: Optional[torch.Tensor],
        ref: torch.Tensor,
    ) -> Optional[torch.Tensor]:
        if not self.use_abs_time_features:
            return None
        B = ref.shape[0]
        if timestamp is None:
            timestamp = torch.zeros(B, dtype=torch.long, device=ref.device)
        else:
            timestamp = timestamp.to(device=ref.device, dtype=torch.long)

        local_ts = timestamp + 8 * 3600
        hour = torch.remainder(torch.div(local_ts, 3600, rounding_mode='floor'), 24)
        weekday = torch.remainder(torch.div(local_ts, 86400, rounding_mode='floor') + 3, 7)

        hour_angle = hour.float() * (2.0 * math.pi / 24.0)
        weekday_angle = weekday.float() * (2.0 * math.pi / 7.0)
        cyclic = torch.stack([
        ...
        token = (
            ...
            + self.abs_time_proj(cyclic)
        )
        token = self.abs_time_norm(token)
    ...
    def _embed_seq_domain(
        ...
        inter_time_bucket_ids: Optional[torch.Tensor] = None,
        time_deltas: Optional[torch.Tensor] = None,
        inter_time_deltas: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        for i in range(S):
            ...
            if (
                self.training
                and self.random_id_mask_prob > 0
                and i < len(is_id)
                and is_id[i]
            ):
                keep = torch.rand_like(vals.float()) >= self.random_id_mask_prob
                vals = vals * keep.long()
        ...
        if self.num_time_buckets > 0:
            ...
            if self.use_inter_time_buckets and inter_time_bucket_ids is not None:
                token_emb = token_emb + self.inter_time_embedding(inter_time_bucket_ids)

        if self.use_continuous_time and time_deltas is not None:
            parts = [torch.log1p(time_deltas.float()).unsqueeze(-1) / 16.0]
            if self.use_inter_time_buckets and inter_time_deltas is not None:
                parts.append(torch.log1p(inter_time_deltas.float()).unsqueeze(-1) / 16.0)
            cont = torch.cat(parts, dim=-1)
            token_emb = token_emb + self.time_mlp(cont)
    ...
    def _build_domain_summary_tokens(
        self,
        target_tokens: torch.Tensor,
        seq_tokens_list: list,
        seq_masks_list: list,
    ) -> list:
        """Build one target-conditioned summary token for each sequence domain."""
        target_query = target_tokens.mean(dim=1)
        query = self.domain_summary_query(target_query).unsqueeze(1)

        summaries = []
        scale = self.d_model ** -0.5
        for seq_tokens, padding_mask in zip(seq_tokens_list, seq_masks_list):
            key = self.domain_summary_key(seq_tokens)
            value = self.domain_summary_value(seq_tokens)

            scores = torch.bmm(query, key.transpose(1, 2)).squeeze(1) * scale
            scores = scores.masked_fill(padding_mask, torch.finfo(scores.dtype).min)
            empty_rows = padding_mask.all(dim=1)
            if empty_rows.any():
                scores = scores.masked_fill(empty_rows.unsqueeze(1), 0.0)

            weights = torch.softmax(scores, dim=-1).unsqueeze(-1)
            summary = torch.sum(weights * value, dim=1)
            if empty_rows.any():
                summary = summary.masked_fill(empty_rows.unsqueeze(1), 0.0)

            gate = self.domain_summary_gate(torch.cat([target_query, summary], dim=-1))
            summary = self.domain_summary_norm(target_query + gate * summary)
    ...
    def _run_multi_seq_blocks(
        ...
        if apply_dropout:
            ...
            if target_tokens is not None:
                target_tokens = self.emb_dropout(target_tokens)
        ...
        domain_summaries = self._build_domain_summary_tokens(
            target_tokens, curr_seqs, curr_masks)
        all_summary = torch.cat(domain_summaries, dim=1)  # (B, S, D)
        output_tokens = torch.cat([all_q, all_summary], dim=1)
        output = output_tokens.reshape(B, -1)  # (B, (Nq+1)*S*D)
        output = self.output_proj(output)  # (B, D)

        return output

    def _prepare_tokens(self, inputs: ModelInput) -> Tuple[torch.Tensor, list, list, list]:
        """Build NS tokens, sequence tokens, masks, and target tokens."""
        ...
        if self.has_xdomain_dense:
            if inputs.xdomain_dense_feats is None:
                B = inputs.user_int_feats.shape[0]
                xdomain_feats = inputs.user_int_feats.new_zeros(
                    B, self.xdomain_dense_dim, dtype=torch.float)
            else:
                xdomain_feats = inputs.xdomain_dense_feats
            xdomain_tok = F.silu(self.xdomain_dense_proj(xdomain_feats)).unsqueeze(1)
            ns_parts.append(xdomain_tok)

        abs_time_tok = self._build_abs_time_token(
            inputs.timestamp, inputs.user_int_feats)
        if abs_time_tok is not None:
            ns_parts.append(abs_time_tok)

        ns_tokens = torch.cat(ns_parts, dim=1)
        target_parts = [item_ns]
        if abs_time_tok is not None:
            target_parts.append(abs_time_tok)
        target_tokens = torch.cat(target_parts, dim=1)

        seq_tokens_list = []
        seq_masks_list = []
        for domain in self.seq_domains:
            inter_buckets = None
            if inputs.seq_inter_time_buckets is not None:
                inter_buckets = inputs.seq_inter_time_buckets.get(domain)
            time_deltas = None
            if inputs.seq_time_deltas is not None:
                time_deltas = inputs.seq_time_deltas.get(domain)
            inter_deltas = None
            if inputs.seq_inter_time_deltas is not None:
                inter_deltas = inputs.seq_inter_time_deltas.get(domain)
            tokens = self._embed_seq_domain(
                inputs.seq_data[domain],
                self._seq_embs[domain], self._seq_proj[domain],
                self._seq_is_id[domain], self._seq_emb_index[domain],
                inputs.seq_time_buckets[domain],
                inter_time_bucket_ids=inter_buckets,
                time_deltas=time_deltas,
                inter_time_deltas=inter_deltas,
            )
            seq_tokens_list.append(tokens)
            mask = self._make_padding_mask(inputs.seq_lens[domain], inputs.seq_data[domain].shape[2])
            seq_masks_list.append(mask)

        return ns_tokens, seq_tokens_list, seq_masks_list, target_tokens

    def forward(self, inputs: ModelInput) -> torch.Tensor:
        """Runs the forward pass of the PCVRHyFormer model."""
        ns_tokens, seq_tokens_list, seq_masks_list, target_tokens = self._prepare_tokens(inputs)

        # 1. Generate independent Q tokens per sequence via MultiSeqQueryGenerator
        q_tokens_list = self.query_generator(ns_tokens, seq_tokens_list, seq_masks_list)

        # 2. Dropout + MultiSeqHyFormerBlock stack + output projection
