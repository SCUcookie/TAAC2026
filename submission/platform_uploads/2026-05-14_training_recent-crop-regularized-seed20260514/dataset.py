...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        train_recent_crop_prob: float = 0.0,
        train_recent_crop_min_ratio: float = 0.5,
    ) -> None:
        """
        Args:
        ...
        self.train_recent_crop_prob = float(train_recent_crop_prob)
        self.train_recent_crop_min_ratio = float(train_recent_crop_min_ratio)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for domain in self.seq_domains:
            ...
            domain_crop_len = max_len
            if (
                self.is_training
                and self.train_recent_crop_prob > 0.0
                and random.random() < self.train_recent_crop_prob
            ):
                min_ratio = min(max(self.train_recent_crop_min_ratio, 0.05), 1.0)
                min_len = max(1, int(round(max_len * min_ratio)))
                if min_len < max_len:
                    domain_crop_len = random.randint(min_len, max_len)
            use_recent_crop = domain_crop_len < max_len
            selected_positions: List[Optional["npt.NDArray[np.int64]"]] = [None] * B
            ts_offs = None
            ts_vals = None
            if ts_ci is not None:
                ...
                if use_recent_crop:
                    ...
                    for i in range(B):
                        ...
                        if rl <= 0:
                            continue
                        k = min(rl, domain_crop_len)
                        ts_row = ts_vals[s:e]
                        valid = ts_row > 0
                        if valid.any():
                            time_diff = np.maximum(row_ts[i, 0] - ts_row, 0)
                            rank_key = np.where(valid, time_diff, np.iinfo(np.int64).max)
                            pos = np.sort(np.argsort(rank_key)[:k]).astype(np.int64)
                        else:
                            pos = np.arange(max(0, rl - k), rl, dtype=np.int64)
            ...
            for c, (offs, vals, vs, ci) in enumerate(col_data):
                for i in range(B):
                    ...
                    if pos is None:
                        ul = min(rl, domain_crop_len)
                        src = vals[s:s + ul]
                    else:
                        ul = min(len(pos), domain_crop_len)
                        src = vals[s + pos[:ul]]
                    out[i, c, :ul] = src
                    if ul > lengths[i]:
                        lengths[i] = ul
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    train_dataset = PCVRParquetDataset(
        ...
        train_recent_crop_prob=train_recent_crop_prob,
        train_recent_crop_min_ratio=train_recent_crop_min_ratio,
    )
