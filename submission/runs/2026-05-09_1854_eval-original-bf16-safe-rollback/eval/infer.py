...
def resolve_amp_dtype(device: torch.device, use_amp: bool, requested_dtype: str) -> Tuple[bool, torch.dtype | None, str]:
    if not use_amp or device.type != "cuda":
        return False, None, "disabled"
    if requested_dtype == "bf16":
        if torch.cuda.is_bf16_supported():
            return True, torch.bfloat16, "bf16"
        print_platform_event("AMP_FALLBACK", {
            "requested": "bf16",
            "fallback": "fp32",
...
def infer() -> Dict[str, Any]:
    ...
    with torch.no_grad():
        ...
        print_platform_event("AMP_CONFIG", {
            ...
            "cuda_bf16_supported": torch.cuda.is_bf16_supported() if device.type == "cuda" else False,
        })
        ...
        def run_pending() -> None:
            ...
            def forward_once(use_amp_now: bool, dtype_now: torch.dtype | None) -> torch.Tensor:
                autocast_kwargs = {
                    "device_type": device.type,
                    "enabled": use_amp_now,
                }
                if dtype_now is not None:
                    autocast_kwargs["dtype"] = dtype_now
                with torch.amp.autocast(**autocast_kwargs):
                    logits_now, _ = model.predict(model_input)
                if device.type == "cuda":
            ...
            t4 = time.time()
            timing_sums["merge_sec"] += t1 - t0
            timing_sums["move_sec"] += t2 - t1
            timing_sums["forward_sec"] += t3 - t2
            timing_sums["post_sec"] += t4 - t3
            ...
            if args.progress_every > 0 and forward_batches % args.progress_every == 0:
                ...
                print_platform_event("INFER_PROGRESS", {
                    ...
                    "timing_avg_sec": {
                        ...
                        "move": round(timing_sums["move_sec"] / forward_batches, 4),
                        "forward": round(timing_sums["forward_sec"] / forward_batches, 4),
                        "post": round(timing_sums["post_sec"] / forward_batches, 4),
                    },
                })
