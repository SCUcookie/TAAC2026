...
def main() -> None:
    ...
    logging.info(
        "DIAG_CONFIG batch_size=%s num_workers=%s seq_max_lens=%s primary_seq_fids=%s use_xdomain=%s use_inter_time=%s use_continuous_time=%s recent_tta=%s recent_tta_weight=%.4f recent_tta_crop_lens=%s",
        batch_size,
        num_workers,
        seq_max_lens,
        primary_seq_fids,
