...
_FALLBACK_TAIL_BLEND_WEIGHT = 0.05
...
def main() -> None:
    ...
    batch_size = min(int(train_config.get('batch_size', _FALLBACK_BATCH_SIZE)), 128)
    num_workers = min(int(train_config.get('num_workers', _FALLBACK_NUM_WORKERS)), 4)
    tail_blend_enabled = _env_bool('EVAL_TAIL_BLEND', True)
    tail_blend_weight = float(os.environ.get(
        'EVAL_TAIL_BLEND_WEIGHT',
        str(_FALLBACK_TAIL_BLEND_WEIGHT),
    ))
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            model_input = _batch_to_model_input(_prefix_batch_without_tail(batch), device)
