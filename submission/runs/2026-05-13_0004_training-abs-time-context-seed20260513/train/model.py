...
class ModelInput(NamedTuple):
    ...
    time_context_dense_feats: Optional[torch.Tensor] = None
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        time_context_dense_dim: int = 0,
        use_absolute_time_context: bool = False,
        ...
        self.use_absolute_time_context = use_absolute_time_context
        self.time_context_dense_dim = time_context_dense_dim
        ...
        self.has_time_context_dense = use_absolute_time_context and time_context_dense_dim > 0
        if self.has_time_context_dense:
            self.time_context_dense_proj = nn.Sequential(
                nn.Linear(time_context_dense_dim, d_model),
                make_norm(d_model, norm_type),
            )
        ...
        self.num_ns = (
            ...
            + (1 if self.has_time_context_dense else 0)
        )
    ...
    def _prepare_tokens(self, inputs: ModelInput) -> Tuple[torch.Tensor, list, list, list]:
        ...
        if self.has_time_context_dense:
            if inputs.time_context_dense_feats is None:
                B = inputs.user_int_feats.shape[0]
                time_context_feats = inputs.user_int_feats.new_zeros(
                    B, self.time_context_dense_dim, dtype=torch.float)
            else:
                time_context_feats = inputs.time_context_dense_feats
            time_context_tok = F.silu(self.time_context_dense_proj(time_context_feats)).unsqueeze(1)
            ns_parts.append(time_context_tok)

        ns_tokens = torch.cat(ns_parts, dim=1)
        target_tokens = item_ns
