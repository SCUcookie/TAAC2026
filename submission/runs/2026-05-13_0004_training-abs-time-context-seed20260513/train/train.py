...
from dataset import FeatureSchema, get_pcvr_data, NUM_TIME_BUCKETS, TIME_CONTEXT_DIM
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--log_every_n_steps', type=int, default=200,
                        help='Emit one concise TRAIN_PROGRESS log line every N training steps '
                             '(0 = disable periodic progress logs)')
    ...
    parser.add_argument('--use_wide_dense_tokens', action='store_true', default=False,
                        help='Split wide dense vectors into dedicated NS tokens')
    parser.add_argument('--wide_dense_threshold', type=int, default=32,
                        help='Dense feature dim above which a separate wide token is created')
    parser.add_argument('--dense_scalar_transform', type=str, default='none',
                        choices=['none', 'signlog'],
                        help='Transform scalar dense features before projection')
    parser.add_argument('--use_xdomain_features', action='store_true', default=False,
                        help='Enable lightweight cross-domain dense statistics')
    parser.add_argument('--primary_seq_fids', type=str, default='',
                        help='Optional primary fid overrides, e.g. seq_a:38,seq_b:69')
    parser.add_argument('--use_inter_time_buckets', action='store_true', default=False,
                        help='Add inter-arrival time bucket embeddings')
    parser.add_argument('--use_continuous_time', action='store_true', default=False,
                        help='Add continuous log time-delta MLP embeddings')
    parser.add_argument('--high_cardinality_mode', type=str, default='zero',
                        choices=['zero', 'hash'],
                        help='How to represent features above emb_skip_threshold')
    parser.add_argument('--hash_num_buckets', type=int, default=1048576,
                        help='Number of buckets for HighCardinalityEmbedding')
    parser.add_argument('--hash_num_hashes', type=int, default=2,
                        help='Number of hash tables for HighCardinalityEmbedding')
    parser.add_argument('--use_target_aware_attn', action='store_true', default=False,
                        help='Refine sequence queries with item target cross-attention')
    parser.add_argument('--use_cross_domain_attn', action='store_true', default=False,
                        help='Apply gated cross-domain query attention before RankMixer')
    parser.add_argument('--seq_topk_mode', type=str, default='recent',
                        choices=['recent', 'semantic'],
                        help='Top-K selection mode for seq_encoder_type=longer')
    parser.add_argument('--ffn_type', type=str, default='gelu',
                        choices=['gelu', 'swiglu'],
                        help='FFN type used in transformer/longer/mixer blocks')
    parser.add_argument('--norm_type', type=str, default='layernorm',
                        choices=['layernorm', 'rms'],
                        help='Normalization type used in model blocks')
    parser.add_argument('--random_id_mask_prob', type=float, default=0.0,
                        help='Training-time masking probability for high-cardinality sequence IDs')
    parser.add_argument('--use_absolute_time_context', action='store_true', default=False,
                        help='Add a compact absolute-time context token derived from row timestamp')
    parser.add_argument('--time_context_utc_offset_hours', type=int, default=8,
                        help='UTC offset used when deriving absolute-time context features')
    ...
    parser.add_argument('--focal_gamma', type=float, default=2.0,
                        help='Focal Loss focusing parameter gamma '
                             '(effective only when --loss_type=focal)')
    parser.add_argument('--focal_alpha_mode', type=str, default='fixed',
                        choices=['fixed', 'auto', 'linear'],
                        help='Focal alpha schedule: fixed, batch auto, or linear decay')
    parser.add_argument('--focal_alpha_start', type=float, default=0.5,
                        help='Starting alpha for --focal_alpha_mode=linear')
    parser.add_argument('--warmup_steps', type=int, default=0,
                        help='Dense AdamW linear warmup steps')
    parser.add_argument('--min_lr_ratio', type=float, default=1.0,
                        help='Cosine decay floor as a ratio of --lr')
    parser.add_argument('--use_ema', action='store_true', default=False,
                        help='Enable EMA weights for validation/checkpointing')
    ...
    args.xdomain_dense_dim = 18 if args.use_xdomain_features else 0
...
def main() -> None:
    ...
    primary_seq_fids = {}
    if args.primary_seq_fids:
        for pair in args.primary_seq_fids.split(','):
            if not pair.strip():
                continue
            k, v = pair.split(':')
            primary_seq_fids[k.strip()] = int(v.strip())
        logging.info(f"Primary seq fid overrides: {primary_seq_fids}")
    ...
    train_loader, valid_loader, pcvr_dataset = get_pcvr_data(
        ...
        use_xdomain_features=args.use_xdomain_features,
        primary_seq_fids=primary_seq_fids,
        use_inter_time_buckets=args.use_inter_time_buckets,
        use_continuous_time=args.use_continuous_time,
        use_absolute_time_context=args.use_absolute_time_context,
        time_context_utc_offset_hours=args.time_context_utc_offset_hours,
    )
    ...
    model_args = {
        ...
        "use_wide_dense_tokens": args.use_wide_dense_tokens,
        "wide_dense_threshold": args.wide_dense_threshold,
        "dense_scalar_transform": args.dense_scalar_transform,
        "xdomain_dense_dim": args.xdomain_dense_dim,
        "time_context_dense_dim": TIME_CONTEXT_DIM if args.use_absolute_time_context else 0,
        "use_absolute_time_context": args.use_absolute_time_context,
        "use_xdomain_features": args.use_xdomain_features,
        "use_inter_time_buckets": args.use_inter_time_buckets,
        "use_continuous_time": args.use_continuous_time,
        "high_cardinality_mode": args.high_cardinality_mode,
        "hash_num_buckets": args.hash_num_buckets,
        "hash_num_hashes": args.hash_num_hashes,
        "use_target_aware_attn": args.use_target_aware_attn,
        "use_cross_domain_attn": args.use_cross_domain_attn,
        "seq_topk_mode": args.seq_topk_mode,
        "ffn_type": args.ffn_type,
        "norm_type": args.norm_type,
        "random_id_mask_prob": args.random_id_mask_prob,
    }
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        focal_alpha_start=args.focal_alpha_start,
